...
def test_deterministic_harness_is_provider_independent():
    ...
    assert result["harness"] == "omicsbase-agent-harness-v2"
    ...
    assert result["native_eval"]["wait_transitions"] == 1
    assert result["native_eval"]["continuation_goal_retained"] is True
    assert result["native_eval"]["total"] >= 13
    assert result["native_eval"]["observations_forwarded"] == result["native_eval"]["total"]
    assert all(
        details["present"]
