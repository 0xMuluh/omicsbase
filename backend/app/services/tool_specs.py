...
NOTE_TOOL_SPECS: tuple[ToolSpec, ...] = (
    ...
    ToolSpec("add_note", "Persist markdown when documenting an important choice, assumption, interpretation, methodology, or explicitly requested explanation. It is optional and is not required for every computation.", _schema({"text": {"type": "string", "description": "The markdown text for the note (2-4 sentences)."}}, required=["text"]), lens="note", kind="async", risk="write", idempotency="non_idempotent"),
