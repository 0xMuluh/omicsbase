...
export OH_VSCODE_PORT="18002"
export OH_VSCODE_BASE_PATH="$VSCODE_BASE_PATH"
# The single route string every static-server instance registers.
# Routes /vscode to Gateway on port 18002
VSCODE_ROUTE="${VSCODE_BASE_PATH}=http://127.0.0.1:18002"
# <<< vscode-config
...
if command -v openhands-agent-server >/dev/null 2>&1; then
  ...
  # System Python package install
  python3 -m openhands.agent_server --port "$AGENT_SERVER_PORT" \
    --import-modules "$AGENT_SERVER_IMPORT_MODULES" &
...
# ── 2b. Start OmicsBase Gateway ──────────────────────────────────────────────
log "Starting OmicsBase Gateway on port 18002..."
export PYTHONPATH="/opt/agent-canvas:${PYTHONPATH:-}"
python3 -m uvicorn gateway.app:app --host 127.0.0.1 --port 18002 &
PIDS+=($!)
...
WAIT_PID2=$!
wait_for_port 18002 "OmicsBase Gateway" 30 &
WAIT_PID3=$!
wait "$WAIT_PID1" "$WAIT_PID2" "$WAIT_PID3"
...
node /opt/agent-canvas/static-server.mjs \
  ...
  --route "/api/conversations=http://127.0.0.1:18002" \
  --route "/api/omicsbase=http://127.0.0.1:18002" \
  --route "/api/vscode=http://127.0.0.1:18002" \
