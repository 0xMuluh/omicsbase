...
class Project(Base):
    ...
    @property
    def report_manifest(self):
        """Return the current rendered-report location for the workspace UI."""
        if not self.project_dir:
            return None
        from app.services.report_artifacts import report_manifest_for_project

        return report_manifest_for_project(self.project_dir)
...
class Job(Base):
    ...
    id = Column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    project_id = Column(String(36), ForeignKey("projects.id"), nullable=False)
    # Compatibility mirror for the durable AgentRun that owns the work.
