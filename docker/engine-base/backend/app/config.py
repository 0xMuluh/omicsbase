...
class Settings(BaseSettings):
    ...
    llm_provider: str = "anthropic"  # "anthropic", "openai", "qwen", "gemini", "openrouter", "orcarouter", "deepseek", "groq", "grok", "xai", "zai", "zenmux", "gmicloud", "ollama"
    ...
    zenmux_api_key: str = Field(default="", repr=False)
    gmi_api_key: str = Field(default="", repr=False)
    bai_api_key: str = Field(default="", repr=False)
    ...
    title_generation_timeout_seconds: float = 2.0
    ...
    vscode_web_url: str = "http://localhost:9888"
    ...
    # Durable agent-run lifecycle. These settings govern ownership and history,
    # not how many steps or tool calls a coding agent may take.
    agent_run_stale_after_seconds: int = 300
    agent_reconciliation_interval_seconds: int = 60
    # Retain durable run envelopes and their event/telemetry history for this
    # many days after terminal completion. Set to 0 to disable automatic purge.
    agent_run_retention_days: int = 90
    agent_run_retention_batch_size: int = 500
    agent_run_retention_interval_hours: int = 24
    ...
    # Coding-agent backend for the project workspace. "native" is the default;
    # the backend-neutral registry allows another workspace-capable agent to
    # be selected without changing project orchestration or the SSE contract.
    coding_agent_backend: str = "native"
    # Legacy setting retained as a fallback for existing deployments.
    ...
    # Optional per-turn OpenCode model variant (for example: low, high, max).
    opencode_agent_variant: str = ""
    # Maximum total staged-file payload sent as native prompt context. Larger
    # data remains in the workspace for targeted agent tool reads.
    opencode_native_attachment_max_bytes: int = 64_000

    # Codex app-server runs as a persistent project-rooted process and uses saved
    # authentication. Provider secrets are deliberately removed from the
    # environment inherited by commands executed inside the workspace.
    codex_bin: str = "codex"
    codex_model: str = ""
    codex_reasoning_effort: str = ""
    codex_approval_mode: str = "auto-accept"
