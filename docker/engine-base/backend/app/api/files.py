...
from app.services.report_artifacts import report_manifest_for_project, resolve_report_file
...
async def run_code_chunk(
    ...
    try:
        from app.services.execution.subprocess import run_command

        cmd = ["quarto", "render", chunk_qmd.name, "--output", "chunk.html", "--output-dir", str(run_dir)]
        success, stdout = await run_command(cmd, cwd=str(code_dir), timeout=CHUNK_TIMEOUT_SECONDS)
...
def serve_report_file(
    ...
    manifest = report_manifest_for_project(project.project_dir)
    try:
        full_path = resolve_report_file(project.project_dir, file_path, manifest)
    except ValueError as exc:
        raise HTTPException(status_code=403, detail="Access denied") from exc
    if not full_path.exists() or not full_path.is_file():
        raise HTTPException(status_code=404, detail=f"File not found: {file_path}")
