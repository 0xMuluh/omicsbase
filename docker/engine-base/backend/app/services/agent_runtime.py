...
def set_agent_state(
    ...
    *,
    ...
    ``clear_summary`` is used when a run has started but has not emitted a
    meaningful observation yet. Keeping the previous run's prose in the
    live-status surface makes the UI claim work that has not been observed.
    """
...
def _summarize_uploaded_file(file_record) -> dict[str, Any]:
    """Keep memory to storage facts; semantic inspection belongs to the agent."""
    summary = file_record.file_summary if isinstance(file_record.file_summary, dict) else {}
    result = {"name": file_record.original_name}
    if summary.get("size_bytes") is not None:
        result["size_bytes"] = summary["size_bytes"]
    if getattr(file_record, "file_path", None):
        result["path"] = file_record.file_path
    return result
