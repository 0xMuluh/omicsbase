...
from app.services.report_artifacts import report_manifest_for_project
...
# contract-less agent path must not require it (agents invent numbered
...
def review_render_output(project_dir: str) -> dict[str, Any]:
    ...
    report_manifest = report_manifest_for_project(base) if execution_contract is None else None
    artifact_relatives = (
        list(execution_contract.artifacts)
        if execution_contract is not None
        else [str(report_manifest["preview_path"])] if report_manifest else ["output/index.html"]
    )
    if execution_contract is None and report_manifest is None:
        return {
            "status": "failed",
            "summary": "Render did not create an output directory or discover a report",
            "checks": checks,
        }
    ...
    report_root = (
        base / str(report_manifest["report_root"])
        if report_manifest is not None
        else output_dir
    )
    report_root_label = str(report_manifest["report_root"]) if report_manifest is not None else "output/"
    checks.extend(
        [
            _check(
                "report_directory",
                report_root.exists(),
                f"{report_root_label} exists" if report_root.exists() else f"{report_root_label} missing",
                severity="warning",
            ),
            _check(
                "html_size",
    ...
    source_dir = (
        ...
        if execution_contract is not None
        else base / str((report_manifest or {}).get("source_dir") or "code")
    )
    source_root = (
        execution_contract.working_directory
        if execution_contract is not None
        else str((report_manifest or {}).get("source_dir") or "code")
    )
    ...
    if execution_contract is None:
        ...
        required_sources = [f"{source_root}/{filename}" for filename in required_names]
    else:
    ...
    if qmd_files:
        has_entrypoint = (
            ...
            or report_manifest is not None
            or len(qmd_files) >= 1
        )
        entry_name = next(
