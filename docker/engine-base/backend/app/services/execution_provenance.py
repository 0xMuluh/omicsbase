...
def write_execution_provenance(
    ...
    *,
    ...
    report_manifest = report_manifest_for_project(root) if contract is None else None
    artifact_paths = list(contract.artifacts) if contract is not None else [str(report_manifest["preview_path"]) if report_manifest else "output/index.html"]
    finished_at = _now()
    record: dict[str, Any] = {
