...
from app.services.execution.subprocess import (
...
def _evaluate_driver(
    *,
    ...
    parts.append(
        ...
        ".note_plots <- 0L\n"
        ".note_tables <- 0L\n"
        ".note_last_display_list <- NULL\n"
        ".note_last_plot_file <- NULL\n"
        ...
        "          .note_merge <- FALSE\n"
        "          if (.note_capture_plots && !is.null(.note_last_display_list)) {\n"
        "            .note_dl <- tryCatch(recordedplot[[1L]], error = function(e) NULL)\n"
        "            if (is.list(.note_dl) && is.list(.note_last_display_list) &&\n"
        "                length(.note_dl) >= length(.note_last_display_list) &&\n"
        "                identical(.note_last_display_list[seq_along(.note_last_display_list)],\n"
        "                          .note_dl[seq_along(.note_last_display_list)])) {\n"
        "              .note_merge <- TRUE\n"
        "            }\n"
        "          }\n"
        "          if (.note_capture_plots) {\n"
        "            if (.note_merge) {\n"
        "              f <- .note_last_plot_file\n"
        "            } else {\n"
        ...
        "            .note_last_plot_file <<- f\n"
        "            .note_last_display_list <<- tryCatch(recordedplot[[1L]], error = function(e) NULL)\n"
        "            if (!.note_merge) .note_emit('plot', .path = f)\n"
        "          } else {\n"
        "            pdf(NULL)\n"
        "            tryCatch({ evaluate::replay(recordedplot) }, finally = dev.off())\n"
        "          }\n"
