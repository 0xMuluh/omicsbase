...
from app.services.execution.subprocess import run_command_sync
...
def import_package_dataset(
    ...
    *,
    ...
    """Export a known R package dataset into project uploads for agent inspection."""
    ...
    with tempfile.TemporaryDirectory(prefix="omicsbase-import-") as tmp:
        ...
        for path in exported:
            ...
            # Imported files remain unclassified; the coding agent inspects them
...
def _register_uploaded_file(db, project, path: Path, *, file_role: str) -> dict[str, Any]:
    ...
    summary = {"name": path.name, "size_bytes": path.stat().st_size, "inspection_status": "deferred"}
