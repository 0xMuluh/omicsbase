...
prepare_clinical_data <- function(path) {
  ...
  # DYAD MAPPING: Propagate maternal traits to pediatric IDs if missing
  # 1. Create DyadID from the suffix (e.g. M3001 -> 001, P1001 -> 001)
  # We use the last 3 digits as they seem to match.
  pheno_data$DyadID <- sub("^(M3|P1|MD|PD)", "", pheno_data$StudyID)
  
  # 2. Extract Maternal Traits (rows where ID starts with M)
  maternal_rows <- pheno_data[grepl("^M", pheno_data$StudyID), ]
  # Remove Child-specific columns from maternal rows if any, but usually they are at Dyad level
  
  # 3. For every respondent, ensure maternal traits are present
  # We join the maternal traits back to the full pheno_data by DyadID
  # First, identify maternal trait columns
  maternal_trait_cols <- c(
    "Intervention", "MAge1", "MprepBMI", "MUniEdu", "MPRSmoke", "PRWLabor", "MModeOfDeliv", "MModeOfDeliv.1",
    "MDietaryPatterns_1", "MDIINormalDiet1", "MDIIDensityDiet1", "MIDQ1", "MIDQ1_categorized",
    "MEPercentProDiet1", "MEPercentCHODiet1", "MEPercentFatDiet1", 
    "MEPercentSFADiet1", "MEPercentMUFADiet1", "MEPercentPUFADiet1", 
    "MEPercentFAn3Diet1", "MEPercentFAn6Diet1", "MEPercentFiballDiet1",
    "MDietaryPatterns_2", "MDIINormalDiet2", "MDIIDensityDiet2", "MIDQ2", "MIDQ2_categorized",
    "MEPercentProDiet2", "MEPercentCHODiet2", "MEPercentFatDiet2", 
    "MEPercentSFADiet2", "MEPercentMUFADiet2", "MEPercentPUFADiet2", 
    "MEPercentFAn3Diet2", "MEPercentFAn6Diet2", "MEPercentFiballDiet2"
  )
  maternal_trait_cols <- maternal_trait_cols[maternal_trait_cols %in% names(maternal_rows)]
  
  maternal_subset <- maternal_rows[, c("DyadID", maternal_trait_cols)]
  maternal_subset <- maternal_subset[!duplicated(maternal_subset$DyadID), ]
  
  # Clean pheno_data of these columns before join to avoid duplicates
  pheno_data_clean <- pheno_data[, !names(pheno_data) %in% maternal_trait_cols]
  pheno_data_final <- left_join(pheno_data_clean, maternal_subset, by = "DyadID")

  rownames(pheno_data_final) <- pheno_data_final$StudyID

  list(
    data = pheno_data_final,
    labels = attr(pheno_data, "variable.labels") # haven might put labels in attributes differently, but let's try
  )
}
...
make_col_data <- function(sample_ids, pheno_data, visit_levels) {
  # Sample IDs may have format: "visit_4.M3001_4" or "M3001_4"
  # Extract the StudyID (e.g., "M3001") and visit number (e.g., "4")
  # Handle both with and without prefix
  
  # Remove prefix if present (e.g., "visit_4." -> "")
...
find_clinical_file <- function(data_dir) {
  ...
  preferred <- sav_files[grepl("maternal", basename(sav_files), ignore.case = TRUE)]
  if (length(preferred) > 1) {
    newest <- preferred[grepl("26062026", basename(preferred))]
    if (length(newest) == 1) return(newest[[1]])
  }
  if (length(preferred) == 1) {
    return(preferred[[1]])
  }
