...
async def list_files(
    ...
    target_path = path if path else project_dir
    try:
        file_list = await call_sync_from_async(runtime.list_files, target_path)
...
async def upload_files(
    ...
    skipped_files = []
    runtime: Runtime = conversation.runtime
    base_dir = getattr(runtime, 'project_dir', runtime.config.workspace_mount_path_in_sandbox)

    for file in files:
        file_path = os.path.join(base_dir, str(file.filename))
        dest_dir = os.path.dirname(file_path)
        temp_dir = tempfile.mkdtemp()
        temp_path = os.path.join(temp_dir, str(file.filename))
        try:
            file_content = await file.read()
            with open(temp_path, 'wb') as f:
                f.write(file_content)
            await call_sync_from_async(runtime.copy_to, temp_path, dest_dir)
