...
if __name__ == '__main__':
    ...
    async def execute_action(action_request: ActionRequest, request: Request):
        ...
        try:
            ...
            if os.environ.get('OMICSBASE_SHARED_RUNTIME') == '1':
                conversation = request.headers.get('X-OmicsBase-Conversation', '')
                project = request.headers.get('X-OmicsBase-Project', '/workspace')
                if not __import__('re').fullmatch(r'[a-zA-Z0-9_-]{1,128}', conversation):
                    raise HTTPException(400, 'Missing conversation identity')
                root = Path('/workspace').resolve()
                work_dir = Path(project).resolve()
                if not work_dir.is_relative_to(root):
                    raise HTTPException(400, 'Invalid project directory')
                async with client.lock:
                    if not hasattr(client, '_conversation_shells'):
                        client._conversation_shells = {}
                    if conversation not in client._conversation_shells:
                        work_dir.mkdir(parents=True, exist_ok=True)
                        client._conversation_shells[conversation] = client._create_bash_session(str(work_dir))
                    original_shell = client.bash_session
                    original_editor = getattr(client, 'file_editor', None)
                    client.bash_session = client._conversation_shells[conversation]
                    client.file_editor = OHEditor(workspace_root=str(work_dir))
                    try:
                        observation = await getattr(client, action.action)(action)
                    finally:
                        client.bash_session = original_shell
                        if original_editor is not None:
                            client.file_editor = original_editor
            else:
    ...
    async def list_files(request: Request):
        ...
        project = request.headers.get('X-OmicsBase-Project', client.initial_cwd)
        base_dir = project if (project and os.path.isabs(project)) else client.initial_cwd
        if path is None:
            full_path = base_dir
