...
assert_filled <- function(x, label) {
  if (is.null(x) || required_marker(x) || optional_marker(x) || identical(x, "")) {
...
read_analysis_plan <- function(path = "config/analysis_plan.R") {
  if (!file.exists(path)) {
    stop(sprintf("Analysis plan not found: %s", path), call. = FALSE)
  }
  ...
  if (!is.list(plan)) {
    stop("analysis_plan must be a list", call. = FALSE)
  }
  plan
}
...
read_table_file <- function(path, na_values = c("", "NA", "NaN")) {
  path <- assert_filled(path, "input path")
  if (!file.exists(path)) stop(sprintf("Input file not found: %s", path), call. = FALSE)
  ext <- tolower(tools::file_ext(path))
  if (ext %in% c("tsv", "txt")) {
    read.delim(path, check.names = FALSE, stringsAsFactors = FALSE, na.strings = na_values)
  } else if (ext == "csv") {
    read.csv(path, check.names = FALSE, stringsAsFactors = FALSE, na.strings = na_values)
  } else {
    stop(sprintf("Unsupported table extension for %s. Use csv or tsv.", path), call. = FALSE)
  }
}
...
require_columns <- function(data, columns, label) {
  columns <- columns[!vapply(columns, optional_marker, logical(1))]
