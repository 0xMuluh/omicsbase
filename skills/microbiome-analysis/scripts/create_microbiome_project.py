#!/usr/bin/env python3
"""Create a cold-start microbiome downstream analysis project from the bundled template."""
import argparse
import re
import shutil
...
def valid_key(value: str) -> str:
    ...
    return key or "microbiome_analysis"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Create a microbiome downstream analysis project skeleton")
    parser.add_argument("target_dir", help="Directory to create or populate")
    parser.add_argument("--title", default="Microbiome analysis", help="Study/report title")
    ...
    print(f"next:    fill config paths, run Rscript code/00_bootstrap_packages.R, then Rscript code/01_prepare_data.R")
