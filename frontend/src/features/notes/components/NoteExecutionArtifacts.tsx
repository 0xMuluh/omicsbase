...
export function NoteExecutionArtifacts({ artifacts, executionId, artifactUrls, artifactErrors, downloadingArtifactId, onDownloadArtifact, open }: NoteExecutionArtifactsProps) {
  if (!artifacts.length) return null;

  const downloadIcon = (artifact: NoteExecutionArtifact) => (
    <Tooltip>
      <TooltipTrigger render={
        <Button size="icon-xs" variant="ghost" aria-label={"Download " + artifactFileName(artifact)} onClick={() => onDownloadArtifact(artifact)} disabled={downloadingArtifactId === artifact.id}>
    ...
    >
      ...
      <div className="space-y-1">
        {artifacts.map((artifact) => {
          const url = artifactUrls[artifact.id];
          const error = artifactErrors?.[artifact.id];
